...
const isBrace = (char) => (braces.includes(char));
...
class TensorNode {
    constructor(symbol, leftChild = null, rightChild = null, op = null) {
        this.symbol = symbol;
...
class FlatTree {
    constructor(rootSymbol, children = [], op = 0) {
...
function getTheLeastPriorityOperator(tokens) {
    let cur = 0;
    const arr = [];
    ...
    const sortedArray = arr.sort((a, b) => a.prior - b.prior);
    return sortedArray[0];
}
function parseTree(tokens, level = 0) {
    if (tokens.length === 1) {
    ...
    const op = getTheLeastPriorityOperator(tokens);
...
function flatTree(tree, level = 0) {
    if (tree === null)
        return null;
...
function distrib(tree) {
    ...
    for (let i = 0; i < tree.children.length - 1; i++) {
        ...
        if (tree.children[i].children.length === 2) {
            ...
            leftChildA = new FlatTree(tree.children[i].rootSymbol, tree.children[i].children.slice(0, tree.children[i].children.length - 1), tree.children[i].op);
        }
        ...
        for (let j = i + 1; j < tree.children.length; j++) {
            ...
            if (equalFlatTree(rightChildA, rightChildB)) {
                const leftChildB = new FlatTree(tree.children[j].rootSymbol, tree.children[j].children.slice(0, tree.children[j].children.length - 1), tree.children[j].op);
            ...
            if (leftChildren.length === tree.children.length) {
                ...
                return new FlatTree('+', [
                    new FlatTree(tree.children[i].rootSymbol, [
                        new FlatTree('+', leftChildren, tree.op + 2),
                        rightChildA
                    ], tree.op + 1),
